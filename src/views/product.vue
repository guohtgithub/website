  ...
  <div>
    ...
    <div class='solution-div row-max'>
      ...
      <a-row :gutter='24' class="pd-48 solution mg0">
        <h2 class="font-color-blue center ptb-48">{{solution.title}}</h2>
        <a-col :xs=24 :sm='10' :offset='!isPC?2:0'  
          v-for="(item,index) in solution.list" :key="item.name"
          class="bg-gray wow"
          :class="index%2?'sliderInRight':'sliderInLeft'"
        >
      ...
      <a-row :gutter='24' class="pd-48 solution mg0">
        ...
        <a-col :xs='24' :sm='6' 
          :offset='!isPC?index == 0 ||index == 3 ?"3":"0":"0"' 
          v-for="(item,index) in actionData.list" 
...
export default {
  data(){
    return{
      ...
      solutionTitle:'',
      selectKey:[],
      descData:[{name:'解决方案'},{name:'功能介绍'},{name:'移动端案例展示'}],
      selectTab:[],
      solution:{
        title:'解决方案',
        list:[
